...
if not capA.isOpened() or not capB.isOpened():
    raise SystemExit(
        "ERROR: could not open one or both camera sources. Check CAMERA_SOURCE_A / CAMERA_SOURCE_B."
    )
...
while True:
    ...
    cv2.putText(
        dispA, f"Device A | {label}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2
    )
    cv2.putText(
        dispB, f"Device B | {label}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2
    )
    ...
    if key == 27:
        ...
        print(
            f"Saved pair {count}: {fnameA}  &  {fnameB}  -- now move the board and press 's' again"
        )
        count += 1
