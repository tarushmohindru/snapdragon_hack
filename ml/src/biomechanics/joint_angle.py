...
if __name__ == "__main__":
    ...
    def check(name, a, b, c, expected_angle):
        ...
        if status == "PASS":
            tests_passed += 1
        print(
            f"[{status}] {name}: expected {expected_angle}°, got {result:.4f}° (error {error:.4f})"
        )

    check("Straight arm (180°)", a=(0, 0, 0), b=(1, 0, 0), c=(2, 0, 0), expected_angle=180.0)

    check("Right angle bend (90°)", a=(0, 1, 0), b=(0, 0, 0), c=(1, 0, 0), expected_angle=90.0)

    check("Fully folded (0°)", a=(1, 0, 0), b=(0, 0, 0), c=(1, 0, 0), expected_angle=0.0)
